...
`sessionUser` SHALL be sourced from the active request context (the `AsyncLocalStorage` populated by `withMiddleware`) and injected automatically by the logging layer, not by each individual call site.
