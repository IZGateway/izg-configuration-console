...
- **GIVEN** the configured develop branch is `develop`
...
- **GIVEN** a hotfix release is requested
...
- **GIVEN** the requested version already has a Git tag
...
- **GIVEN** release versions must use `X.Y.Z` format
...
- **GIVEN** the latest existing semver tag is `v1.18.0`
...
- **GIVEN** a standard release for version `1.19.0` is requested
...
- **GIVEN** a hotfix release for version `1.15.1` is requested
...
- **GIVEN** the triggering branch is named `hotfix/1.15.1`
...
- **GIVEN** only standard and hotfix release types are recognized
...
- **GIVEN** a release is being cut from `develop`
...
- **GIVEN** no pull requests have merged since the previous semver tag
...
- **GIVEN** the release dependency tree is audited before image publication or branch merges
...
- **GIVEN** the release dependency tree is audited
...
- **GIVEN** the release run executes its unit test suite
...
- **GIVEN** the release inputs and branch satisfy validation
...
- **GIVEN** dry-run is disabled and all registries are configured
...
- **GIVEN** a standard release for version `1.18.0` is being cut
...
- **GIVEN** `develop` already has a package version before the hotfix run
...
- **GIVEN** the release run has not updated develop
...
- **GIVEN** the run has not created persistent state
...
- **GIVEN** a successful non-dry-run release has published the image
...
- **GIVEN** a successful non-dry-run release has dispatched the advisory scan
