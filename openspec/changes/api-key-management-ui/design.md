...
**Decision:** persist `ApiKeyCredential.useTypes` as a deduped **List**. The lib-dynamodb v3
DocumentClient returns Sets as native JS `Set`s (no unmarshall-to-array option), and the
design ER diagram itself shows `+List useTypes`; a List avoids Set round-trip footguns.
Reads use `filter(isValidUseType)`. `useTypes` is a server-side property (Hub reads it by
`jti`), **not** a JWT claim.
