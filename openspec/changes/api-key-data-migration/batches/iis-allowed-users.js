...
// iis-allowed-users.js — Load IIS AllowedUser records into DynamoDB via BatchWriteItem.
// Node replacement for iis-allowed-users.sh (see that file's header for background).
//
// Usage:
//   node iis-allowed-users.js --table <dynamodb-table-name> [--profile <aws-profile>]
...
const USAGE = 'Usage: node iis-allowed-users.js --table <dynamodb-table-name> [--profile <aws-profile>]';
const CSV = path.join(__dirname, 'denormalized', 'allowed-users-iis.csv');
...
async function main() {
  ...
  console.log(`Loading IIS AllowedUsers → table: ${args.table}`);
  ...
  console.log(`Done. ${ok} IIS AllowedUser records written to ${args.table}. Failures: ${failed}.`);
