...
#
# Usage:
#   ./iis-allowed-users.sh --table <dynamodb-table-name> [--profile <aws-profile>]
#
# Example:
#   ./iis-allowed-users.sh --table izgateway-dev-test --profile cdc
#   ./iis-allowed-users.sh --table izgw-hub --profile production

set -uo pipefail

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
CSV="${SCRIPT_DIR}/denormalized/allowed-users-iis.csv"
...
echo "Done. $COUNT IIS AllowedUser records written to $TABLE. Failures: $FAILURES."