...
for ID in 5 6 8 9 11 12 17 18 19 20 21 23 25 28 35 36 38 39 40 41 42 43 50 51 53 54 56 58 60 61 62; do
  aws dynamodb update-item --table-name "$TABLE" \
    ...
    --expression-attribute-values '{":aut":{"SS": ["PROVIDER", "PUBLIC_HEALTH"]},":ut":{"SS": ["PUBLIC_HEALTH"]}}'
done
...
for ID in 7 13 15 22 29 30 32 33 37 52 59; do
  aws dynamodb update-item --table-name "$TABLE" \
    ...
    --expression-attribute-values '{":aut":{"SS": ["PROVIDER"]}}'
done
...
for ID in 3 10 27 31 34 57 63; do
  aws dynamodb update-item --table-name "$TABLE" \
    --key '{"entityType":{"S":"Jurisdiction"},"sortKey":{"S":"'$ID'"}}' \
    --update-expression 'SET allowedUseTypes = :aut, useTypes = :ut' \
    --expression-attribute-values '{":aut":{"SS": ["PATIENT", "PROVIDER", "PUBLIC_HEALTH"]},":ut":{"SS": ["PUBLIC_HEALTH"]}}'
done
...
for ID in 4 44 45 46 47 48 49; do
  aws dynamodb update-item --table-name "$TABLE" \
    --key '{"entityType":{"S":"Jurisdiction"},"sortKey":{"S":"'$ID'"}}' \
    --update-expression 'SET allowedUseTypes = :aut, useTypes = :ut' \
    --expression-attribute-values '{":aut":{"SS": ["PUBLIC_HEALTH"]},":ut":{"SS": ["PUBLIC_HEALTH"]}}'
done
...
for ID in 16 24 26; do
  aws dynamodb update-item --table-name "$TABLE" \
    --key '{"entityType":{"S":"Jurisdiction"},"sortKey":{"S":"'$ID'"}}' \
    --update-expression 'SET allowedUseTypes = :aut' \
    --expression-attribute-values '{":aut":{"SS": ["PATIENT", "PROVIDER"]}}'
done
...
for ID in 1; do
  aws dynamodb update-item --table-name "$TABLE" \
    --key '{"entityType":{"S":"Jurisdiction"},"sortKey":{"S":"'$ID'"}}' \
    --update-expression 'SET allowedUseTypes = :aut' \
    --expression-attribute-values '{":aut":{"SS": ["PATIENT", "PROVIDER", "PUBLIC_HEALTH"]}}'
done
...
for ID in 14; do
  aws dynamodb update-item --table-name "$TABLE" \
    --key '{"entityType":{"S":"Jurisdiction"},"sortKey":{"S":"'$ID'"}}' \
    --update-expression 'REMOVE allowedUseTypes'
done
...
for ID in 55; do
  aws dynamodb update-item --table-name "$TABLE" \
    --key '{"entityType":{"S":"Jurisdiction"},"sortKey":{"S":"'$ID'"}}' \
    --update-expression 'SET allowedUseTypes = :aut' \
    --expression-attribute-values '{":aut":{"SS": ["PROVIDER", "PUBLIC_HEALTH"]}}'
done
