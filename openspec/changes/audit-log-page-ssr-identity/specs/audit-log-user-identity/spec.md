...
### Requirement: Server-side page-load reads carry sessionUser

Authenticated page data reads performed in `getServerSideProps` SHALL execute inside the request context so their log events — including downstream DB-layer logs — carry the `sessionUser` object, the same way `/api/*` requests do. This applies to the pages that read data directly server-side: `manageconnections`, `changerequest/[...slug]`, `testreport`, and `test/[...slug]` (the connection test). Existing identity fields on those lines (for example the connection test's `userContext`) SHALL be preserved (additive).

#### Scenario: Page-load DB read is attributed

- **WHEN** an authenticated user loads a page whose `getServerSideProps` reads data directly from the database (e.g. Manage Connections reading destinations / change requests)
...
`sessionUser` SHALL be sourced from the active request context (the `AsyncLocalStorage` in `src/lib/Context.ts`) and injected automatically by the logging layer, not by each individual call site. The request context SHALL be established both by `withMiddleware` (for `/api/*` routes) and by authenticated `getServerSideProps` data reads (page server-side reads, wrapped via the shared context builder), so server-side page-load reads are attributed the same way as API requests.
