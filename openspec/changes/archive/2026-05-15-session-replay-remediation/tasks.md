...
- [x] 3.3 Verify no Okta access token appears in NextAuth JWT cookie after fix:
...
- [x] 3.4 Regression test: log in, navigate admin pages, confirm `isAdmin` and
...
- [x] 4.7 Verify `boundPublicKey` is present in the session after `bind-session` completes:
      the session cookie is a JWE (A256GCM) and cannot be decoded client-side.
...
- [x] 4.8 Attack simulation: capture `storageState` after login, attempt replay in a
...
- [x] 4.10 Deploy to dev; repeat attack simulation against dev environment
- [x] 4.11 Update CVSS note in IGDD-2893 — WebCrypto binding eliminates attack for CC
