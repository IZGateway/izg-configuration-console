...
The `sessionId` field SHALL be an opaque identifier generated by the Configuration Console at sign-in (e.g. a UUID), persisted on the session token. It SHALL remain stable for the lifetime of a login session and take a new value on each new login. It SHALL NOT be a secret and SHALL NOT be replayable to gain access. The system SHALL NOT log any raw JWT, access token, ID token, or session cookie value.

> The Okta ID token `jti` is explicitly excluded as the source — it is not pivotable in the Okta System Log and identifies the token, not the session. The Okta `sid` claim (the only value that would enable a direct Okta-log pivot) is deferred because it requires an Okta SLO config change; if later adopted it becomes the source of `sessionId` with no other change (see design.md D2).

#### Scenario: Session identifier is generated, not a token claim

- **WHEN** a user signs in
- **THEN** `sessionId` is set to a Configuration-Console-generated opaque value, not to any raw Okta token or token claim such as `jti`

### Requirement: User-initiated logs support indirect Okta correlation

Because the generated `sessionId` does not appear in Okta's logs, every log event emitted during an authenticated user interaction SHALL also carry the fields needed to correlate it back to the originating Okta login event: `userId` (the Okta `sub`), `auth_time` (the Okta login timestamp, captured at sign-in), and the source IP address.

#### Scenario: Correlation fields present on a user-initiated log

- **WHEN** an authenticated user interaction produces a log event
- **THEN** the event includes `userId` (Okta `sub`), `auth_time`, and the source IP, enabling correlation to the Okta System Log via user + login time + IP

#### Scenario: auth_time captured at sign-in
