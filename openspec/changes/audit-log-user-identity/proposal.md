...
- Standardize the identity block shape across all logs:
  ...
  ```
- Introduce a **`sessionId`** derived from the Okta ID token's `jti` claim, captured once at sign-in. It is stable for the life of a login session, is not a secret, and cannot be replayed. Also capture `auth_time` to bridge to Okta System Log (correlation via `sub` + `auth_time` + source IP).
...
- **Code:**
  ...
  - `src/pages/api/auth/[...nextauth].ts` — extract `jti` (sessionId) and `auth_time` from the Okta ID token in the `jwt` callback and persist on the next-auth token.
...
- **Auth / security:** Reads additional Okta ID-token claims (`jti`, `auth_time`); no change to authentication, authorization, jurisdiction scoping, or encrypted fields. `sessionId` is deliberately a non-replayable token identifier — no raw JWT, access token, or session cookie value is ever logged.
...
- **Open assumption:** Okta System Log is not reliably searchable by ID-token `jti`; correlation to Okta relies on `sub` + `auth_time` + source IP. Worth confirming with the Okta administrator, but not a blocker.
