...
// Test 12-a-i
test('Verify Connections Table, for Admin, for all row count options', async ({
  page,
}) => {
...
test('Verify connections table for non-admin user', async ({ page }) => {
  await loginToOkta(
    page,
    process.env.OKTA_NONADMIN_USERNAME,
    process.env.OKTA_NONADMIN_PASSWORD,
...
test('Verify unauthorized user cannot login', async ({ page }) => {
  ...
  await page
    ...
    .fill('THIS IS A BOGUS PASSWORD')
  await page.locator('[type="submit"]').click()

  await expect(page.locator('.okta-form-infobox-error')).toContainText(
    'Unable to sign in'
  )
})
