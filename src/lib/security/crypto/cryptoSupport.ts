...
const KEY_NAME = process.env.NEXT_PUBLIC_DDB_ENCRYPTION_KEYNAME || null
 ...
 */
async function getKeyFromSecretsManager(
  ...
  if (currentResponse.SecretString) {
    ...
    if (currentKey.length !== KEY_LENGTH) {
      throw new Error(
        `Current secret key length is ${currentKey.length}, expected ${KEY_LENGTH}`
      )
    }
  }
  ...
  try {
    ...
    if (previousResponse.SecretString) {
      ...
      if (previousKey.length !== KEY_LENGTH) {
        throw new Error(
          `Previous secret key length is ${previousKey.length}, expected ${KEY_LENGTH}`
        )
      }
    }
  } catch (e) {
