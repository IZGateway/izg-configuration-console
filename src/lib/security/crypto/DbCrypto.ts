  ...
  for (const dest of destinations) {
    try {
      ...
      if (typeof password === 'string' && password) {
        ...
        const updated: Destination = { ...dest, password: encrypted }
        await dbClient.getRepository().updateDestination(updated) // TODO: Log updater info
      }
    } catch (error) {
...
export async function resetDb(dbClient: DbClient): Promise<void> {
  ...
  for (const dest of destinations) {
    ...
    if (
      ...
      success =
        success && (await dbClient.getRepository().updateDestination(updated)) // TODO: Log updater info
    }
  }
}
