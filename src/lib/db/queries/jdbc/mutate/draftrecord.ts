...
const upsertDraftRecord = async (changeRequestData: any) => {
  if (_.isEmpty(changeRequestData.password)) {
    const currentDestPassword = await prismacontext.prisma.$queryRaw<
      any[]
    >`SELECT password FROM destinations where dest_id=${changeRequestData.dest_id} and dest_type=${changeRequestData.dest_type}`
...
export default upsertDraftRecord
